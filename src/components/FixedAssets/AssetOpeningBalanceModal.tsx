...
const AssetOpeningBalanceModal: React.FC<AssetOpeningBalanceModalProps> = ({
  ...
  const handleAdd = () => {
    ...
    onAdd({ type: 'Existing', date, value });
    setShowForm(false);
    setDate('');
    setValue('');
    setErrors({});
          ...
          <table className="opening-balance-table">
            ...
            <tbody>
              {openingBalances.length === 0 ? (
                <tr>
                  <td colSpan={3} className="opening-balance-empty">No opening balances added yet.</td>
                </tr>
              ) : (
                openingBalances.map((ob, i) => (
                  <tr key={i}>
                    <td>{formatDate(ob.date)}</td>
                    <td>{ob.value ? `$${Number(ob.value).toLocaleString()}` : '—'}</td>
                    <td>
